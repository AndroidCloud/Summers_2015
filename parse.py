...
if __name__=="__main__":
    svo = SVO()
    for i in range(1970):
	lines = [line.rstrip('\r\n') for line in open('./Videos/vid' + `i+1` + '.txt')];
	for j in range(len(lines)):
		print lines[j];
		try:
	    		tree = svo.get_parse_tree(lines[j]);
	    		val = svo.process_parse_tree(tree);
			print val
			target = open('./SVO_Triplets/'+ 'vid' + `i+1` + '.txt', 'a');
			if val:
				if val[0].has_key('subject_info'):
					sub = val[0]['subject_info']['subject'];
					if sub:
						target.write("Subject : " +  sub);
						target.write("  ");
				if val[0].has_key('predicate_info'):
					verb = val[0]['predicate_info']['predicate'];
					if verb:
						target.write("Verb : " + verb);
						target.write("  ");
				if val[0].has_key('object_info'):
					obje = val[0]['object_info']['object'];
					if obje:
                				target.write("Object : " + obje);
